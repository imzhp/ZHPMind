...
- 按章节显示，个人感想直接展开，对应原文紧随其后、默认折叠；只有同章节、同 range 且唯一匹配时才配对。无感想的划线统一收在章节末尾的折叠摘录中。同步数量、来源链接和更新日期放页尾，不重复笔记标题。没有感想就不补写。
...
排版规则升级后，可在原命令末尾加 `--format-only`：只用已保存的来源数据重排，不读取授权、不联网，保留原更新日期。仍须通过同步区校验，原文、感想和同步区外内容不变，成功返回 `formatted`。布局版本写入本机状态，后续无变化的同步不重写。
